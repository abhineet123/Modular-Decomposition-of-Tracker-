...
class FilterParams:
    def __init__(self, type, params):
        ...
        for key in params.keys():
            ...
            self.params[key]=Param(name=key, id=params[key]['id'], base=default_vals['base'], mult=default_vals['mult'],
        ...
        if type == 'gabor':
            ...
            self.apply = lambda img: cv2.Laplacian(img, -1, ksize=self.params['ksize'].val,
                                                   scale=self.params['scale'].val, delta=self.params['delta'].val)
        elif type == 'sobel':
            print 'Initializing Sobel filter'
            self.apply = lambda img: cv2.Sobel(img, -1, ksize=self.params['ksize'].val, scale=self.params['scale'].val,
                                               delta=self.params['delta'].val, dx=self.params['dx'].val, dy=self.params['dy'].val)
            ...
            self.apply = lambda img: cv2.Scharr(img, -1, scale=self.params['scale'].val,
            ...
            self.apply = lambda img: cv2.Canny(img, threshold1=self.params['low_thresh'].val,
            ...
            def applyLoG(img):
                gauss_img = cv2.GaussianBlur(img, ksize=(self.params['gauss_ksize'].val, self.params['gauss_ksize'].val),
